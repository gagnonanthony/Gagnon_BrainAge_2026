...
option_list <- list(
  ...
  make_option(c("-o", "--output"), type="character", default="trajectoryPredictors.csv",
  ...
  make_option(c("-c", "--covariates"), type="character", default=NULL,
              help="Comma-separated list of covariates to include in the model", metavar="character"),
  make_option(c("-s", "--subject_id"), type="character", default="sid",
              help="Subject ID variable name [default= %default]", metavar="character"),
  make_option(c("-p", "--method"), type="character", default="twoStageJoint",
              help="Method for externVar function (default is none)", metavar="character"),
...
# Use the externVar function to assess predictors of latent class membership
if (!is.null(opt$covariates)) {
  covariates <- unlist(strsplit(opt$covariates, ","))
  formula_str <- paste("~ ", paste(covariates, collapse = " + "))
  formula <- as.formula(formula_str)
  message("Evaluating covariates: ", paste(covariates, collapse = ", "))
  extern_mod <- externVar(
    model,
    classmb = formula,
    longitudinal = list(m1, m2, m3, m4),
...
# Save the p-values, coefficients, and odds ratios to a CSV file.
if (!is.null(opt$covariates)) {
  sink("/dev/null") # Suppress console output
  summary_extern <- summary(extern_mod)
  sink() # Re-enable console output
  write.csv(summary_extern, file.path(opt$output), row.names=TRUE)
  message("externVar results saved to CSV.")
}